"""
Find best balance between volume and profitability for SOL.
"""
import sys
import os
...
print('Loading SOL data...', flush=True)
df = pd.read_csv('data/SOLUSDT_1m.csv', parse_dates=['open_time'], index_col='open_time')
...
for i, combo in enumerate(combinations):
    ...
    if (i + 1) % 50 == 0:
        print(f'Progress: {i+1}/{len(combinations)}', flush=True)

results.sort(key=lambda x: x['score'], reverse=True)

print('\n' + '='*60)
print('TOP 10 PROFITABLE HIGH-VOLUME SOL CONFIGURATIONS')
print('='*60)
...
if best:
    ...
    with open('config/optimized_params_sol.json', 'w') as f:
        json.dump(optimized, f, indent=2)

    daily_volume = best['trades_per_day'] * 2000  # Assuming $2000 avg position
    print(f"\nBEST SOL CONFIG SAVED:")
    ...
    print("\nNo profitable SOL configurations found!")
